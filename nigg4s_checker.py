"""Nigg4s Checker by Notchitting | t.me/Niggasphobia"""
...
Logger.log(f"Nigg4s Checker started at {time()}")
...
def TITLE_SPINNER():
    ...
    TITLE = ["Nigg4s", "Avaible : {WORKS}", "Taken : {TAKEN}", "Requests : {REQUESTS}", "RPS : {RPS}", "Elapsed : {ELAPSED}s"]
    while True:
        for i in TITLE:
            edited = i          
            for _ in range(50):
...
def WEBHOOK_PROCESSOR():
    ...
    while True:
        ...
        if len(names_diff) > 1 and last_send_time-time() < 5: #if we get hits frequently and user is not scaning for super rare names we switch to batch mode
            ...
            json = {"content": "\n".join(payload), 'username': 'Nigg4s', 'avatar_url': 'https://upload.wikimedia.org/wikipedia/commons/thumb/9/95/Burning_Yellow_Sunset.jpg/1280px-Burning_Yellow_Sunset.jpg'}
            ...
            json = {"content": message.replace("<name>", name).replace("<time>", str(hittime)).replace("<elapsed>", str(round(current_time - start_time))).replace("<RPS>", str(RPS)), 'username': 'Nigg4sChecker', 'avatar_url': 'https://upload.wikimedia.org/wikipedia/commons/thumb/9/95/Burning_Yellow_Sunset.jpg/1280px-Burning_Yellow_Sunset.jpg'}
            x = CLOUDCHECKER.session.post(
                url=webhook,
                json=json
            )
